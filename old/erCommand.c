...
static void decodeBrief(k3FreqMem * fmem) {
...
static void decodeRaw(k3FreqMem * fmem) {
...
static void decodeVerbose(k3FreqMem * fmem) {
...
static char * printLabel(char label[], char * asciiLabel) {
...
static int ant1(char state)      { return !ant2(state); }
...
static int repeaterOffset(char * rptrOffset) {
...
static int modeCw(char state)      { return stox(state) == 0; } 
static int modeLsb(char state)     { return stox(state) == 1; } 
static int modeUsb(char state)     { return stox(state) == 2; } 
static int modeData(char state)    { return stox(state) == 3; } 
static int modeAm(char state)      { return stox(state) == 4; } 
static int modeFm(char state)      { return stox(state) == 5; } 

static int nbOn(char state)      { return stox(state) & 0x2; } 

/* modeAlt 'state' is vfoAflags1 */
static int modeAlt(char state)   { return stox(state) & 0x4; } 

static int ant2(char state)      { return stox(state) & 0x8; }

static int preampOn(char state)  { return stox(state) & 0x8; }

static int attOn(char state)     { return stox(state) & 0x1; }
static int rxAntOn(char state)   { return stox(state) & 0x8; }

static int splitOn(char state)   { return stox(state) & 0x1; } 

static int nrOn(char state)      { return stox(state) & 0x4; } 

static float plTone(char * plHex) {
 ...
 */
static int checksum(char asciiHex[], int from, int to, char * checksumAscii) {
...
static int checksumOrig(char wtf[]) {
  unsigned int b=0;
  int i;
  char *t;
  unsigned int c=0;
